...
static void screen_write_ch(char ch)
{
    ...
    if (ch == '\n')
    {
        current_running[core_id]->cursor_x = 0;
        current_running[core_id]->cursor_y++;
    }
    if (ch == '\b' || (int)ch == 127)
    {
        if(current_running[core_id]->cursor_x > 0){
            (current_running[core_id]->cursor_x)--;
            new_screen[SCREEN_LOC(current_running[core_id]->cursor_x, current_running[core_id]->cursor_y)] = ' ';
        }
    }
    else
    {
        new_screen[SCREEN_LOC(current_running[core_id]->cursor_x, current_running[core_id]->cursor_y)] = ch;
        current_running[core_id]->cursor_x++;
    }
}
...
void screen_clear(void)
{
    ...
    current_running[core_id]->cursor_x = 0;
    current_running[core_id]->cursor_y = 0;
    screen_reflush();
}
...
{
    int core_id = get_current_cpu_id();
    current_running[core_id]->cursor_x = x;
    current_running[core_id]->cursor_y = y;
...
void screen_reflush(void)
{
    ...
    vt100_move_cursor(current_running[core_id]->cursor_x, current_running[core_id]->cursor_y);
}
