...
{
    if((uintptr_t)str & KERNEL_VA_FLAG){
        str = kva2pa((uintptr_t)str);
    }
    else{
        core_id = get_current_cpu_id();
        str = get_pa_of((uintptr_t)str, current_running[core_id]->pgdir);
    }
...
static inline void bios_logging(char *str)
{
    ...
    call_jmptab(QEMU_LOGGING, (long)str, 0, 0);
}
