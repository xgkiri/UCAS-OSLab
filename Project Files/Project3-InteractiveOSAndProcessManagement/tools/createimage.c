...
#define TASK_INFO_ADDR (OS_SIZE_LOC - 4)
#define BOOT_LOADER_SIG_1 0x55
#define BOOT_LOADER_SIG_2 0xaa
#define TASK_MAXNUM 16
#define ONE_TASK_INFO_SIZE 32
#define NBYTES2SEC(nbytes) (((nbytes) / SECTOR_SIZE) + ((nbytes) % SECTOR_SIZE != 0))

/* TODO: [p1-task4] design your own task_info_t */
typedef struct {
    char name[24];//24bytes
    int size;    //4bytes
    int offset;  //4bytes
} task_info_t;   //32bytes totall
                           ...
                           short tasknum, FILE * img, int *offset)
{
    ...
    fseek(img, TASK_INFO_ADDR, SEEK_SET);
    fwrite(offset, 4, 1, img);
    fseek(img, TASK_INFO_ADDR - 4, SEEK_SET);
    fwrite(&tasknum, 2, 1, img);
    fseek(img, *offset, SEEK_SET);
    fwrite(taskinfo, ONE_TASK_INFO_SIZE, tasknum, img);
